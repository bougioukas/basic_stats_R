...
## Measures of diagnostic accuracy and ROC curve

Cut-off points often are selected in order to calculate the measures of diagnostic accuracy. Based on previous studies, the cut-off points for a positive response are:
...
dat <- dat %>% 
  mutate(IPAG_cat = cut(IPAG, c(3, 17, 35), labels=c("-","+"), 
...
**A. Measures of diagnostic accuracy**
...
**B. The area under the ROC IPAG curve**

We can plot the sensitivity on the y-axis and 1-specificity (false positives) values on the x-axis for all possible cut-off points of the diagnostic test.

The area under the ROC curve (AUC of ROC), also called the c-statistic, is a widely used measure of overall performance.

First, let's get sensitivity, specificity and the AUC (95% CI) of ROC for the IPAG questionnaire. We will use these values to provide additional information about the data being displayed:
...
Now, we are ready to visualize the ROC IPAG plot (@fig-roc1):

```{r}
#| warning: false
#| label: fig-roc1
#| fig-width: 7
#| fig-cap: The ROC curve of IPAG questionnaire with the optimal cut-off point.
...
The AUC of IPAG ROC equals to 0.799 (95% CI: 0.769 - 0.829) which indicates a reasonable diagnostic test. Obviously, a test which is perfect at discriminating between those with disease and those without disease has an AUC = 1 (i.e. the ROC curve gets to the upper left-hand corner).
...
**B. The area under the ROC COPDPS curve**

Similarly, let's get sensitivity, specificity and the AUC (95% CI) of ROC for the COPDPS questionnaire to provide additional information in the plot:
...
The ROC IPAG plot (@fig-roc2) follows:
...
The AUC of COPDPS ROC equals to 0.791 (95% CI: 0.760 - 0.821) which is close to the value 0.799 of AUC of IPAG ROC.

## Compare two ROC Curves

**A. Graphically comparison of ROC curves**
...
**B. Compare two AUCs using the DeLong' s test**
