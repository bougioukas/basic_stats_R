...
The data set *meldata* has 250 patients (rows) and includes seven variables (columns). We are intrested in the character (`<chr>`) `ulcer` variable and the character (`<chr>`) `status` variable which should be converted to factor (`<fct>`) variables using the `convert_as_factor()` function as follows:
...
Finnaly, we can also reverse the odds ratio:
