...
The difference in location between the two distributions can be tested using a rank test such as Wilcoxon-Mann-Whitney (WMW):
...
Based on the p \<0.001, we reject the null hypothesis, the result (the median of the difference[^wmw-1] = 18.95, 95% CI: 13.7 to 24) is significant.

[^wmw-1]: **Note:** the estimator for the difference in location parameters does not estimate the difference in medians (a common misconception) but rather the median of the difference between the two samples.
...
Hence, there is evidence that the urinary $\beta$ thromboglobulin excretion is higher in diabetic group, median = 29.2 (IQR: 27.1, 34.8) pg/ml, as compared to non-diabetic group, 10.9 (8.3, 14.8) pg/ml. The median of the difference (18.95, 95% CI: 13.7 to 24) is significant (p \<0.001).
