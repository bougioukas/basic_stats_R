...
**Density plot** is also used to present the distribution of a continuous variable and it is a variation of the histogram allowing for smoother distributions^[Density curves are usually scaled such that the area under the curve equals one.] (@fig-density1). In this case, `geom_density()` function is used for displaying density plot.
...
[^descriptive-1]: An outlier is an observation that is significantly distant from the main body the data. We say any value outside of the following interval is an outlier: $$(Q_1 - 1.5 \times IQR, \ Q_3 + 1.5 \times IQR)$$

```{r}
#| warning: false
#| label: fig-boxplot1
#| fig-width: 6
#| fig-cap: Box plots of (a) age and (b) QRS variables stratified by sex.
#| fig-subcap: 
...
#| label: fig-raincloud
#| fig-width: 6
#| fig-cap: A raincloud plot of (a) age and (b) QRS variables stratified by sex.
#| fig-subcap: 
