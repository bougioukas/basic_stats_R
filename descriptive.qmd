...
1. `bmi_cat` is an ordered variable so the order of the levels have to be specified explicitly in the `factor()` function.
...
Let $x_1, x_2,...,x_{n-1}, x_n$ be a set of *n* measurements. The arithmetic sample mean or average, $\bar{x}$, is simply the sum of the observations divided by their number *n*, thus:
