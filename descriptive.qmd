...
Standard deviation of a data set is the square root of the calculated sample variance:

$$sd= \sqrt{s^2} = \sqrt\frac{\sum_{i=1}^{n}(x_{i}-\bar{x})^2}{n-1}$$
...
The Range is the difference between the minimum (lowest) and maximum (highest) values.One of the greatest disadvantages of using range as a method of dispersion is that range is sensitive to outliers in the data. 
