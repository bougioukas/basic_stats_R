...
We might have noticed that the categorical variables `sex` and `bmi_cat` are recognized of character `<chr>` type. We can use the `convert_as_factor()` function to convert the variables to factors:

```{r}
arrhythmia <- arrhythmia %>% 
  convert_as_factor(sex, bmi_cat)
```
...
Similarly, we can create the frequency table for the `bmi_cat` variable in a decreasing order of frequencies:
...
Numeric data are described with two main types of summary measures:
...
::: {#exercise-joins .callout-tip}
## Summary statistics: 1 variable by group

::: panel-tabset
## dplyr

```{r}
age_by_sex_summary <- arrhythmia %>%
  group_by(sex) %>% 
  dplyr::summarise(
    n = n(),
    min = min(age, na.rm = TRUE),
...
arrhythmia %>% 
  ...
  geom_vline(aes(xintercept = median(age)), color = "grey30", linetype = "dashed", size = 0.8) +
  theme_minimal() +
    labs(y = "Frequency") +
